  ...
  const onSubmit = async (values: TransactionFormValues) => {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        ...
        errorMessage = `Failed to schedule transaction: ${message || err.statusText}`;
      }
                  ...
                  <li key={tx.id} className="flex justify-between items-center border-b pb-2 text-sm">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {tx.created_at && tx.delay_seconds && tx.delay_seconds > 0 && CANCELLABLE_STATUSES.includes(tx.status as any) && (
                          <CountdownTimer executionTime={new Date(new Date(tx.created_at).getTime() + tx.delay_seconds * 1000)} />
                        )}
                      </div>
                    </div>
                    {tx.status && CANCELLABLE_STATUSES.includes(tx.status as any) && (
